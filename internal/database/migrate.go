...
// Migrate aplica as migrations embutidas (NNN_descricao.up.sql) que ainda nao
// constam em tse_migrations. Cada migracao roda em transacao propria.
...
func ensureMigrationsTable(ctx context.Context, db DB) error {
	_, err := db.Exec(ctx, `
		CREATE TABLE IF NOT EXISTS tse_migrations (
...
func pendingMigrations(ctx context.Context, db DB) ([]string, error) {
	...
	rows, err := db.Query(ctx, `SELECT versao FROM tse_migrations`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func apply(ctx context.Context, db DB, version string) error {
	...
	if _, err := tx.Exec(ctx, `INSERT INTO tse_migrations (versao) VALUES ($1)`, version); err != nil {
		return err
	}
